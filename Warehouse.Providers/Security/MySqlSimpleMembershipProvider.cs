...
namespace Warehouse.Providers.Security
{
    ...
    public class MySqlSimpleMembershipProvider : ExtendedMembershipProvider
    {
        ...
        public override bool EnablePasswordRetrieval
        {
            get
            {
                return !InitializeCalled && PreviousProvider.EnablePasswordRetrieval;
            }
        }
        ...
        public override bool EnablePasswordReset
        {
            get
            {
                return !InitializeCalled && PreviousProvider.EnablePasswordReset;
            }
        }
        ...
        public override bool RequiresQuestionAndAnswer
        {
            get
            {
                return !InitializeCalled && PreviousProvider.RequiresQuestionAndAnswer;
            }
        }
        ...
        public override bool RequiresUniqueEmail
        {
            get
            {
                return !InitializeCalled && PreviousProvider.RequiresUniqueEmail;
            }
        }
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (config.Count > 0)
            {
                ...
                {
                    throw new ProviderException(String.Format(CultureInfo.CurrentCulture, Resources.Security.SimpleMembership_ProviderUnrecognizedAttribute, attribUnrecognized));
                }
            }
        }

        internal static bool CheckTableExists(MySqlMembershipContext db, string tableName)
        {
        ...
        private static void CreateOAuthTokenTableIfNeeded(MySqlMembershipContext db)
        {
            // NOTICE: It does not needed in Entity Framework Code First
        ...
        public int GetUserId(string userName)
        {
            ...
            return GetUserId(NewMySqlMembershipContext, userName);
        }

        internal static int GetUserId(MySqlMembershipContext db, string userName)
        {
        ...
        public override string CreateAccount(string userName, string password, bool requireConfirmationToken)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                ...
                db.Memberships.Add(new Model.Entities.Membership
                {
                    UserId = uid,
        ...
        private void CreateUserRow(MySqlMembershipContext db, string userName, IDictionary<string, object> values)
        {
            ...
            if (values != null && values.Count > 0)
            {
                ...
                var userProfileExType = typeof(UserProfile);
                ...
                addMethod.Invoke(userProfileExDbSet, new[] { userProfileEx });		// invoke add UserProfile inherited class object
        ...
        private void CreateUserRowByDatabase(MySqlMembershipContext db, string userName, IDictionary<string, object> values)
        {
            var newUserProfile = new UserProfile
            {
                UserName = userName,
        ...
        private static bool SetPassword(MySqlMembershipContext db, int userId, string newPassword)
        {
            string hashedPassword = Crypto.HashPassword(newPassword);
            if (hashedPassword.Length > 128)
            {
                throw new ArgumentException(Resources.Security.SimpleMembership_PasswordTooLong);
            }
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                ...
                return userId == -1
                    ? null
                    : new MembershipUser(System.Web.Security.Membership.Provider.Name, username, userId, null, null,
                        null, true, false, DateTime.MinValue, DateTime.MinValue, DateTime.MinValue, DateTime.MinValue,
                        DateTime.MinValue);
            }
        }
        ...
        private static int GetPasswordFailuresSinceLastSuccess(MySqlMembershipContext db, int userId)
        {
            var membership = db.Memberships.SingleOrDefault(x => x.UserId == userId);
        ...
        private bool CheckPassword(MySqlMembershipContext db, int userId, string password)
        {
            var hashedPassword = GetHashedPassword(db, userId);
        ...
        private string GetHashedPassword(MySqlMembershipContext db, int userId)
        {
            var membership = db.Memberships.SingleOrDefault(x => x.UserId == userId);

            return membership != null ? membership.Password : null;
        }

        // Ensures the user exists in the accounts table
        private int VerifyUserNameHasConfirmedAccount(MySqlMembershipContext db, string userName, bool throwException)
        {
            var userId = GetUserId(db, userName);
            ...
            if (result == 0)
            {
                if (throwException)
                {
                    throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, Resources.Security.Security_NoAccountFound, userName));
                }
        ...
        public override string GeneratePasswordResetToken(string userName, int tokenExpirationInMinutesFromNow)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                var userId = VerifyUserNameHasConfirmedAccount(db, userName, throwException: true);
                var membership = db.Memberships.SingleOrDefault(x => x.UserId == userId);

                if (membership != null)
                {
                    var token = string.Empty;    //membership.PasswordVerificationToken;
        ...
        public override bool IsConfirmed(string userName)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                var userId = VerifyUserNameHasConfirmedAccount(db, userName, throwException: false);
                return (userId != -1);
            }
        }
        ...
        internal void ValidateTable()
        {
            using (var db = NewMySqlMembershipContext)
            {
                ...
                if (db.Database.CompatibleWithModel(false) == false)
                    throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, Resources.Security.Security_FailedToFindUserTable, "Unknown"));
            }
        }
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                ...
                return userId != -1 && CheckPassword(db, userId, password);
            }
        }
        ...
        public override string GetOAuthTokenSecret(string token)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                ...
                return oAuthToken != null ? oAuthToken.Secret : string.Empty;
            }
        }
        ...
        public override void StoreOAuthRequestToken(string requestToken, string requestTokenSecret)
        {
            ...
            using (var db = NewMySqlMembershipContext)
            {
                ...
                if (insert != 1)
                {
                    throw new ProviderException(Resources.Security.SimpleMembership_FailToStoreOAuthToken);
                }
            }
        }
