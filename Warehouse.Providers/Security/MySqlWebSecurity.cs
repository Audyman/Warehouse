    ...
    public static class MySqlWebSecurity
    {
        ...
        internal static void PreAppStartInit()
        {
            ...
            if (ConfigUtil.MySqlSimpleMembershipEnabled)
            {
                ...
                var builtInMembership = Membership.Providers[builtInMembershipProviderName] as MembershipProvider;
                if (builtInMembership != null)
                {
                    ...
                    Membership.Providers.Add(mySqlSimpleMembership);
                }

                Roles.Enabled = true;
                const string builtInRolesProviderName = "MySqlSimpleRoleProvider";
                var builtInRoles = Roles.Providers[builtInRolesProviderName] as RoleProvider;
        ...
        {
            var provider = Membership.Provider as MySqlSimpleMembershipProvider;

            if (provider == null)
            {
                throw new InvalidOperationException(Resources.Security.Security_NoExtendedMembershipProvider);
            }
        ...
        {
            var connect = new DatabaseConnectionInfo { ConnectionStringName = connectionStringName };
        ...
        {
            var connect = new DatabaseConnectionInfo { ConnectionString = connectionString, ProviderName = providerName };
            InitializeProviders(connect);
        }

        private static void InitializeProviders(DatabaseConnectionInfo connect)
        {
            var extendedMembership = Membership.Provider as MySqlSimpleMembershipProvider;
